...
describe('Stock e2e test', () => {
  ...
  it('should load Stocks', async () => {
    ...
    expect(await stockComponentsPage.getTitle()).to.eq('yahooApp.stock.home.title');
  ...
  it('should load create Stock page', async () => {
    ...
    expect(await stockUpdatePage.getPageTitle()).to.eq('yahooApp.stock.home.createOrEditLabel');
  ...
  it('should create and save Stocks', async () => {
    ...
    await promise.all([stockUpdatePage.setNameInput('name'), stockUpdatePage.userSelectLastOption()]);

    expect(await stockUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
  ...
  it('should delete last Stock', async () => {
    ...
    expect(await stockDeleteDialog.getDialogTitle()).to.eq('yahooApp.stock.delete.question');
